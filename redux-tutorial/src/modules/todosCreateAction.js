...
//2. 액션 생성 함수 정의
export const changeInputCreateAction = createAction(CHANGE_INPUT, (input) => input);
...
const todosCreateAction = handleActions(
  {
    //handleActions에서는 액션이 필요한 추가 데이터를 모두 payload라는 이름으로 사용한다
    //리듀서를 구현할 때 action.payload사용
    //비구조화 할당으로 payload의 이름을 새로 정할 수 있다
